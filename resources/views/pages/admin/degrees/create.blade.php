...
<div class="flex-grow px-8 py-4">
    <h1 class="text-grey-dark text-center">اضافة رتبة</h1>

    @component('components.admin.form-card')

    @slot('action')
        {{route('admin.degrees.create')}}
    @endslot
    @slot('method')
    POST
    @endslot
   ...
   <div class="px-4 mb-8">
        <label class="font-semibold block text-center text-blue-darker mb-4 " for="abbreviation">الاختصار</label>
        <input type="text" name="abbreviation" class="border border-grey-light border-solid w-4/5 block mx-auto px-4 py-4 rounded" id="abbreviation">
    </div>   
    <div class="px-4 mb-8">
