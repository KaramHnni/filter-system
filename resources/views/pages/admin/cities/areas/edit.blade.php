...
@section('page-title')
تعديل منطقة@endsection
@section('main-content')
<div class="flex">
<div class="flex-grow px-8 py-4">
    <h1 class="text-grey-dark text-center">تعديل منطقة</h1>

    @component('components.admin.form-card')

    @slot('action')
        {{route('admin.cities.areas.edit' ,['city_id' =>$area->city->id ,'area_id' => $area->id])}}
    @endslot
    @slot('method')
    POST
    @endslot
    ...
    <input type="text" name="name" value="{{$area->name}}" class="border border-grey-light border-solid w-4/5 block mx-auto px-4 py-4 rounded " id="name">
   </div>
   <div class="px-4 mb-8">
        <label class="font-semibold block text-center text-blue-darker mb-4 " for="slug">الشعار</label>
        <input type="text" name="slug" value="{{$area->slug}}" class="border border-grey-light border-solid w-4/5 block mx-auto px-4 py-4 rounded" id="slug">
    </div>
    <div class="px-4 mb-8">
